defmodule GettextSigils.ModifiersTest do
  ...
  describe "KeywordModifier.init/1" do
    ...
    test "accepts :preprocess and :postprocess as remote function captures" do
      assert {:ok, opts} =
               KeywordModifier.init(
                 preprocess: &Function.identity/1,
                 postprocess: &String.upcase/1
               )

      assert Keyword.fetch!(opts, :preprocess) == (&Function.identity/1)
      assert Keyword.fetch!(opts, :postprocess) == (&String.upcase/1)
    end

    test "rejects :preprocess / :postprocess anonymous functions" do
      assert {:error, %NimbleOptions.ValidationError{} = error} =
               KeywordModifier.init(postprocess: fn s -> s end)

      assert Exception.message(error) =~ "remote function capture"
    end

    test "rejects :preprocess / :postprocess non-function values" do
      assert {:error, %NimbleOptions.ValidationError{} = error} =
               KeywordModifier.init(postprocess: :not_a_fn)

      assert Exception.message(error) =~ "remote function capture"
    end
  end

  describe "KeywordModifier preprocess/postprocess callbacks" do
    # Module-level helpers used as remote captures below. KeywordModifier
    # rejects anonymous functions, so the tests need named module functions.
    defmodule ExampleFns do
      @moduledoc false
      def trim_msgid({msgid, bindings}), do: {:ok, {String.trim(msgid), bindings}}
      def error_msgid(_parsed), do: {:error, "preprocess failed"}
      def upcase(string), do: {:ok, String.upcase(string)}
      def error_string(_string), do: {:error, "postprocess failed"}
    end

    test "preprocess/2 calls the :preprocess function when present" do
      {:ok, opts} = KeywordModifier.init(preprocess: &ExampleFns.trim_msgid/1)

      assert KeywordModifier.preprocess({"  hi  ", []}, opts) == {:ok, {"hi", []}}
    end

    test "preprocess/2 is a no-op when :preprocess is absent" do
      {:ok, opts} = KeywordModifier.init(domain: "errors")

      assert KeywordModifier.preprocess({"hi", []}, opts) == {:ok, {"hi", []}}
    end

    test "preprocess/2 propagates {:error, reason} from the user function" do
      {:ok, opts} = KeywordModifier.init(preprocess: &ExampleFns.error_msgid/1)

      assert KeywordModifier.preprocess({"hi", []}, opts) == {:error, "preprocess failed"}
    end

    test "postprocess/2 calls the :postprocess function when present" do
      {:ok, opts} = KeywordModifier.init(postprocess: &ExampleFns.upcase/1)

      assert KeywordModifier.postprocess("hi", opts) == {:ok, "HI"}
    end

    test "postprocess/2 is a no-op when :postprocess is absent" do
      {:ok, opts} = KeywordModifier.init(domain: "errors")

      assert KeywordModifier.postprocess("hi", opts) == {:ok, "hi"}
    end

    test "postprocess/2 propagates {:error, reason} from the user function" do
      {:ok, opts} = KeywordModifier.init(postprocess: &ExampleFns.error_string/1)

      assert KeywordModifier.postprocess("hi", opts) == {:error, "postprocess failed"}
    end
  end
